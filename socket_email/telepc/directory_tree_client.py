"""Directory tree UI to browse and transfer files between client and server."""
...
def abs_path(file_name):
    """Return absolute path for an asset, supporting PyInstaller bundles.

    Parameters
    ----------
    file_name: str
        File name located under the local ``assets`` directory.
    """
...
def listDirs(client, path):
    """Request child entries under ``path`` from the server.

    Returns a list of tuples ``[(name, is_dir), ...]`` or an empty list when
    the server reports an error. UI error dialogs are shown for invalid states.
    """
...
class DirectoryTree_UI(Canvas):
    """Canvas-based widget to explore server directories and transfer files."""

    def __init__(self, parent, client):
        """Create the directory tree view bound to an active socket client."""
    ...
    def insert_node(self, parent, text, abspath, isFolder):
        """Insert a node in the tree and mark it expandable when a folder."""
    ...
    def open_node(self, event):
        """Populate children for the expanded node by querying the server."""
    ...
    def select_node(self, event):
        """Update the path text field with the currently selected item."""
    ...
    def deleteTree(self):
        """Clear the tree view and reset the selection state."""
        self.currPath = " "
        self.path.config(state="normal")
        self.path.delete("1.0", tk.END)
        self.path.config(state="disable")
    ...
    def showTree(self):
        """Request the root directory list and populate the tree view."""
        self.deleteTree()
    ...
    def copyFileToServer(self):
        """Upload a file from the local machine to the selected server folder."""
    ...
    def copyFileToClient(self):
        """Download the selected server file to a chosen local directory."""
    ...
    def deleteFile(self):
        """Delete the selected server file and display the outcome."""
    ...
    def back(self):
        """Placeholder hook used by parent to override navigation behavior."""
        return
