"""Utilities for UI actions: query details and download images/avatars."""

import tkinter as tk

from PIL import Image, ImageTk

from socket_place.client.data import get_avt, get_detail_info, get_img
from socket_place.client.pages import Page, create_frames, show_first_frame
...
def query_one_place_clicked(id, name):
    """Open a dialog showing detailed information and the avatar image."""
    w = tk.Toplevel()
    ...
    lbl_name = tk.Label(
        master=frm_name, text="Tên địa điểm: " + place_name, bg="#98D8B2", fg="red"
    )
    ...
    str_coordinate = (
        "(" + str(place_coordinate[0]) + ", " + str(place_coordinate[1]) + ")"
    )
    lbl_coordinate = tk.Label(
        master=frm_coordinate, text="Tọa độ: " + str_coordinate, bg="#D8AD98", fg="red"
    )
    ...
    lbl_description = tk.Label(
        master=frm_description,
        text="Mô tả: " + place_description,
        bg="#D8989C",
        fg="red",
        anchor="w",
        justify=tk.LEFT,
...
def download_images_one_place_clicked(id, name, images_num):
    """Open a dialog to page through all gallery images for the place."""
    w = tk.Toplevel()
...
def download_all_avatars_clicked(id_list, name_list):
    """Open a dialog displaying avatars for all places in a grid."""
    w = tk.Toplevel()
